...
2. store.extraValues.sharingSetting.use === true 로직 자체를 store 도메인에서 함수로 제공하여 사용
...
위와 같은 코드는 `디미터 법칙`을 위반한 코드입니다. 디미터 법칙을 위반하면 `메시지 체인(Message Chain)` 이라는 악취가 나게됩니다.
